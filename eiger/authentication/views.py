...
from eiger.authentication.forms import (
    CreateClimberForm,
    CreateTrainerForm,
    LoginClimberForm,
    LoginTrainerForm,
...
def climber_index_view(request: HttpRequest) -> HttpResponse:
    ...
    return render(
        request=request,
        template_name='pages/climbers/index.html',
        context={
...
def climber_registration_view(request: HttpRequest) -> HttpResponse:
    ...
    if not form.is_valid():
        ...
        return redirect(to='climber-index')
...
@require_GET
def trainer_index_view(request: HttpRequest) -> HttpResponse:
    logger.debug('Received a request for a user accessing the index view.')
    registration_form: Type[CreateTrainerForm] = CreateTrainerForm
    login_form: Type[LoginTrainerForm] = LoginTrainerForm
    ...
    if login_data := context.get('login'):
        login_form: LoginTrainerForm = login_form()  # type: ignore[no-redef]
        login_form.cleaned_data = login_data.get('cleaned_data', {})
        login_form._errors = login_data.get(  # type: ignore[attr-defined]
            'errors'
        )
...
@require_POST
def trainer_registration_view(request: HttpRequest) -> HttpResponse:
    logger.debug('Received a request for a user sending registration data.')
    ...
    if not form.is_valid():
        ...
        return redirect(to='trainer-index')
...
@require_POST
def trainer_login_view(request: HttpRequest) -> HttpResponse:
    logger.debug('Received a request for a user sending login data.')
    form = LoginTrainerForm(
        request,
        data=request.POST,
    )
    if not form.is_valid():
